...
#+BEGIN_SRC asm :tangle stage0.s
  entry_header "HIDE", HIDE
    fw forth_interpreter
    /* Not using back-pointer, but will have to make XT be the start pointer */
...
{{{cstart}}}
#+BEGIN_SRC asm :tangle stage0.s
  entry_header "ENTRY->XT", ENTRY_TO_EXECUTION_TOKEN /* TODO: We could make xt be the entry start, and use EXECUTE/COMPILE, */
    fw forth_interpreter
