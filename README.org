...
This project is split into the following stages, and tracks A through
to H; where each track represents an optional feature of the full
system. The optimisation is a soft-dependency, in that each track has
optimisations, but you don't need to select all tracks to have
optimisations, if you select a subset then only the optimisations for
that track get compiled.
...
.fdef "2DUP", TWO_DUP, OVER, OVER, EXIT
   #+end_src
- REPL :: A REPL which compiles text onto said virtual machine. This
